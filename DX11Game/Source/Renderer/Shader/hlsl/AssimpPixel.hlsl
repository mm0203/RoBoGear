...
Texture2D	 rampTex			 : register(t5); // トゥーンテクスチャ
...
float4 main(VS_OUTPUT input) : SV_Target0
{
	...
	{
		// 視点へのベクトル
		float3 V = normalize(g_vCameraPos.xyz - input.PosForPS);
		// ハーフベクトル
	 ...
	 //     Diff = g_vLightAmbient.rgb * g_Ambient.rgb +
		   ...
		   // 鏡面反射色
		  Spec = Spec * g_vLightSpecular.rgb * pow(saturate(dot(N, H)), g_Specular.a);
		  Diff += Spec;
	  }
